...
const files = readdir(viewPath)
	.filter(view => view.indexOf('errors/' === -1))
	.sort((a, b) => {
...
for (const { stone, html } of Array.from(tests).sort()) {
	...
	test(view, async t => {
		...
		const [rendered, target] = await Promise.all([
			app.view.engine
				.render(view, app.config.get('view.data'))
				.then(rendered => rendered.trim()),
		...
		t.fail(
			`Rendered Stone does not match expecting HTML:\nStone: '${rendered}'\nTarget: '${target}'\nCompiled: '${compiled}'`,
		)
	})
}
