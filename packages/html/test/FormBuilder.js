...
test('closing form', async t => {
	t.is((await formBuilder()).close().toString(), '</form>')
})

test('form label', async t => {
	const form = await formBuilder()

	t.is(form.label('foo', 'Foobar').toString(), '<label for="foo">Foobar</label>')
...
test('form input', async t => {
	...
	t.is(form.input('text', 'foo').toString(), '<input name="foo" type="text" />')
...
test('file not filled', async t => {
	t.is((await formBuilder()).file('img').toString(), '<input name="img" type="file" />')
})
...
test('form text', async t => {
	...
	t.is(form.input('text', 'foo').toString(), form.text('foo').toString())

	t.is(form.text('foo', 'foobar').toString(), '<input name="foo" type="text" value="foobar" />')
...
test('form repopulation with mix of arrays and objects', async t => {
	...
	t.is(form.text('letters[1]').toString(), '<input name="letters[1]" type="text" value="b" />')
})

test('form password', async t => {
	const form = await formBuilder()

	t.is(form.password('foo').toString(), '<input name="foo" type="password" value="" />')
...
test('form hidden', async t => {
	...
	t.is(form.hidden('foo').toString(), '<input name="foo" type="hidden" />')
...
test('form email', async t => {
	...
	t.is(form.email('foo').toString(), '<input name="foo" type="email" />')

	t.is(form.email('foo', 'foobar').toString(), '<input name="foo" type="email" value="foobar" />')
...
test('form tel', async t => {
	...
	t.is(form.tel('foo').toString(), '<input name="foo" type="tel" />')

	t.is(form.tel('foo', 'foobar').toString(), '<input name="foo" type="tel" value="foobar" />')
...
test('form number', async t => {
	...
	t.is(form.number('foo').toString(), '<input name="foo" type="number" />')

	t.is(form.number('foo', 1).toString(), '<input name="foo" type="number" value="1" />')
...
test('form date', async t => {
	...
	t.is(form.date('foo').toString(), '<input name="foo" type="date" />')
...
test('form time', async t => {
	...
	t.is(form.time('foo').toString(), '<input name="foo" type="time" />')
...
test('form file', async t => {
	...
	t.is(form.file('foo').toString(), '<input name="foo" type="file" />')
...
test('form textarea', async t => {
	...
	t.is(form.textarea('foo').toString(), '<textarea name="foo" cols="50" rows="10"></textarea>')
...
test('select', async t => {
	...
	t.is(
		form
			.select('size', { L: 'Large', S: 'Small' }, null, {
				class: 'class-name',
				id: 'select-id',
			})
			.toString(),
	...
	t.is(
		form.select('select-name-id', [], null, { name: 'select-name' }).toString(),
...
test('form with optional placeholder', async t => {
	...
	t.is(
		form
			.select('size', { L: 'Large', S: 'Small' }, null, { placeholder: 'Select One...' })
			.toString(),
	...
	t.is(
		form
			.select('size', { L: 'Large', S: 'Small' }, 'L', { placeholder: 'Select One...' })
			.toString(),
...
test('form select range', async t => {
	...
	t.is(range.substring(0, 53), '<select name="dob"><option value="1900">1900</option>')

	t.is(range.indexOf('<option value="2013">2013</option>') >= 0, true)
})
...
test('form checkbox', async t => {
	...
	t.is(form.input('checkbox', 'foo').toString(), '<input name="foo" type="checkbox" />')

	t.is(form.checkbox('foo').toString(), '<input name="foo" type="checkbox" value="1" />')
...
test('form radio', async t => {
	...
	t.is(form.input('radio', 'foo').toString(), '<input name="foo" type="radio" />')

	t.is(form.radio('foo').toString(), '<input name="foo" type="radio" value="foo" />')
...
test('form radio repopulation', async t => {
	...
	t.is(form.radio('radio', 2, true).toString(), '<input name="radio" type="radio" value="2" />')
})

test('form submit', async t => {
	const form = await formBuilder()

	t.is(form.submit('foo').toString(), '<input type="submit" value="foo" />')
...
test('form button', async t => {
	...
	t.is(form.button('foo').toString(), '<button type="button">foo</button>')
...
test('reset input', async t => {
	t.is((await formBuilder()).reset('foo').toString(), '<input type="reset" value="foo" />')
})

test('image input', async t => {
	const url = 'http://grind.rocks/'

	t.is((await formBuilder()).image(url).toString(), `<input src="${url}" type="image" />`)
})

test('form color', async t => {
	const form = await formBuilder()

	t.is(form.color('foo').toString(), '<input name="foo" type="color" />')
