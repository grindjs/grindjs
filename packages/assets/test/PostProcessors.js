...
async function make(processor, file, hook = () => {}) {
	...
	return {
		...
		processor: new processor(
			app,
			app.config.get('assets.should_optimize'),
			app.config.get('assets.source_maps'),
		),
	}
}
...
test('css-postcss', async t => {
	const { processor, file, contents } = await make(
		CssPostCssPostProcessor,
		'css/test-autoprefix.css',
	)
	const css = await processor.process(file, null, contents)
...
test('babel-minify', async t => {
	...
	t.is(
		js.toString().trim(),
		'export class Test{test(){return document.body.getElementById("test")}};',
	)
})
