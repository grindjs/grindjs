...
class WatchNeverFiredError extends Error {
	constructor() {
		super('Watch never fired')
	}
}
...
test('ensure clear cache', async t => {
	...
	const writeJson = json => {
		return FS.writeFile(file, `module.exports = ${JSON.stringify(json)}`).then(() =>
			delay(1000),
		)
	}
