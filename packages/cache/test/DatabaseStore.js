...
function store() {
	...
	return cache.store.db.schema
		.createTable('cache', table => {
			table.string('key').unique()
			table.text('value')
			table.datetime('expires_at')
		})
		.then(() =>
			cache.store.db('cache').insert(
				{
					key: 'test',
...
test('set', t => {
	return store()
		...
		.then(store => store.get('test2'))
		.then(value => t.is(value, 'testing'))
})

test('buffer', t => {
	return store()
		.then(store => store.set('buffer', new Buffer('testing')).then(() => store))
		.then(store => store.get('buffer'))
		.then(value => {
...
test('object', t => {
	return store()
		...
		.then(store => store.get('object'))
		.then(value => t.is(value.a.b, 'c'))
})

test('set expired', t => {
	return store()
		.then(store => store.set('test3', 'testing', { ttl: -1 }).then(() => store))
		.then(store => store.get('test3'))
		.then(value => t.is(value, null))
})
